...
def start_detection(left_reps, right_reps, rest_duration):
    ...
    while True:
        ...
        if resting:
            ...
            if detector.checkInsideBox(img, box):
                ...
                elif time.time() - time_inside_box > 3:
                    inside_box = True
            ...
            if inside_box:
                ...
                if (current_side == 'left' and current_reps >= left_reps) or (
                    ...
                    print(f"Completed {current_reps} {current_side.capitalize()} Arm Reps. Rest for {rest_duration} seconds.")
            else:
                tracking_started = False
        ...
        if time_inside_box is not None and not inside_box:
            ...
            cv2.putText(img, f"Starting in {countdown}s", (w // 2 - 100, h // 2), cv2.FONT_HERSHEY_PLAIN, 3, (0, 255, 255), 3)
